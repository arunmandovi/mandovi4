...
public interface MGAProfitRepository extends JpaRepository<MGAProfit, Integer> {
    ...
    @Query("""
            ...
            """)
    public List<MGAProfit> getMGAProfitMonthYear (@Param("months") List<String> months,@Param("years") List<String> years,
                                                  @Param("financialYears") List<String> financialYears );

    @Transactional
    @Modifying
    @Query("DELETE FROM MGAProfit m WHERE m.month = :month AND m.year = :year")
